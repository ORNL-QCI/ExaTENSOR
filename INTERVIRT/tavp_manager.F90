...
!TYPES:
 ...
 !TAVP-MNG decomposer:
        type, extends(ds_unit_t), private:: tavp_mng_decomposer_t
         ...
         class(tens_cache_t), pointer, private:: arg_cache=>NULL()  !tensor argument cache pointer
         type(list_bi_t), private:: subinstruction_list             !list of newly created subinstructions
         type(list_iter_t), private:: sub_list                      !iterator for <subinstruction_list>
         type(list_bi_t), private:: create_list                     !list of prologue TENSOR_CREATE instructions for new output subtensors
         type(list_iter_t), private:: crt_list                      !iterator for <create_list>
...
!Reserve a bytecode buffer:
         call this%bytecode%reserve_mem(ier,MAX_BYTECODE_SIZE,MAX_BYTECODE_INSTR); if(ier.ne.0.and.errc.eq.0) errc=-38
 ...
 !Decode new bytecode:
             do i=1,num_packets
              ...
              if(opcode.eq.TAVP_INSTR_CTRL_STOP) then !copy the CTRL_STOP instruction into own port 0
               if(i.ne.num_packets.and.errc.eq.0) then; errc=-22; exit wloop; endif !STOP must be the last instruction in the packet
...
!Reserve a bytecode buffer:
         call this%bytecode%reserve_mem(ier,MAX_BYTECODE_SIZE,MAX_BYTECODE_INSTR); if(ier.ne.0.and.errc.eq.0) errc=-76
!Initialize queues and ports:
         call this%init_queue(this%num_ports,ier); if(ier.ne.DSVP_SUCCESS.and.errc.eq.0) errc=-75
!Initialize the located instruction list iterator:
         ier=this%loc_list%init(this%located_list); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) errc=-74
!Initialize the deferred instruction list iterator:
         ier=this%def_list%init(this%deferred_list); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) errc=-73
!Initialize the missing-operand instruction list iterator:`Not used, everything goes into def_list
         ier=this%mis_list%init(this%missing_list); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) errc=-72
!Initialize the control list:
         ier=this%ctrl_list%init(this%control_list); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) errc=-71
!Set up a dummy instruction (RESUME):
         call tens_instr_dummy%tens_instr_ctor(TAVP_INSTR_CTRL_RESUME,ier,iid=int(role_rank,INTL)) !special dummy instruction with id = role_rank
         if(ier.ne.0.and.errc.eq.0) errc=-70
!Check ring topology:
         if(this%ring_send.eq.role_rank.and.this%ring_recv.eq.role_rank) then !root TAVP-MNG (no ring)
          ...
          if(this%ring_send.eq.role_rank.or.this%ring_send.lt.0.or.&
            &this%ring_recv.eq.role_rank.or.this%ring_recv.lt.0) errc=-69 !trap
         endif
!Set up tensor argument cache and wait on other TAVP units:
         tavp=>NULL(); dsvp=>this%get_dsvp(); select type(dsvp); class is(tavp_mng_t); tavp=>dsvp; end select
         if(associated(tavp)) then
          this%arg_cache=>tavp%tens_cache
          call tavp%sync_units(errc,ier); if(ier.ne.DSVP_SUCCESS.and.errc.eq.0) errc=-68
         else
          this%arg_cache=>NULL(); if(errc.eq.0) errc=-67
         endif
!Work loop:
         num_loc_instr=0; num_def_instr=0
         ier=timer_start(MAX_LOCATE_WAIT_TIME,loc_wait); if(ier.ne.TIMERS_SUCCESS.and.errc.eq.0) errc=-66
         active=((errc.eq.0).and.(this%ring_comm.ne.MPI_COMM_NULL)); stopping=(.not.active)
         wloop: do while(active)
 !Absorb new tensor instructions from udecoder (port 0) to the end of the main queue:
          ier=this%iqueue%reset_back(); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-65; exit wloop; endif
          ier=this%flush_port(0); if(ier.ne.DSVP_SUCCESS.and.errc.eq.0) then; errc=-64; exit wloop; endif
 !Insert the header dummmy instruction, which will tag the bytecode, into the locating list:
          ...
          if(ier.eq.GFC_IT_EMPTY) then
           ier=this%loc_list%append(tens_instr_dummy); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-62; exit wloop; endif
           ier=this%loc_list%reset_back(); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-61; exit wloop; endif
          endif
 !Move the leading subset of new tensor instructions from the main queue into the locating list:
          ier=this%iqueue%reset(); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-60; exit wloop; endif
          ier=this%iqueue%get_status()
          if(ier.eq.GFC_IT_ACTIVE) then !main queue is not empty
           ier=this%iqueue%move_list(this%loc_list,MAX_LOCATE_NEW_INSTR,n) !at most MAX_LOCATE_NEW_INSTR tensor instructions will be moved to the locating list
           if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-59; exit wloop; endif
 ...
 !Move a limited number of tensor instructions from the deferred list back into the locating list:
          ...
          ier=this%def_list%reset(); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-56; exit wloop; endif
          ier=this%def_list%get_status()
          if(ier.eq.GFC_IT_ACTIVE.and.num_def_instr.lt.MAX_LOCATE_DEF_INSTR) then
          ...
          if(time_is_off(loc_wait,ier,destroy=.TRUE.)) then
           ier=timer_start(MAX_LOCATE_WAIT_TIME,loc_wait); if(ier.ne.TIMERS_SUCCESS.and.errc.eq.0) then; errc=-54; exit wloop; endif
          else !timer has not expired yet
           if(num_loc_instr.lt.MIN_LOCATE_INSTR) cycle wloop !there is no enough instructions to initiate locating rotations
          endif
 !Perform a full rotation of the tensor instructions being located at the given NAT level:
          if(ring_exists) then
           rot_num=0 !rotation number
           rloop: do !rotations
  !Encode tensor instructions from the locating list into bytecode (also deactivate own control instructions):
            ier=this%loc_list%reset(); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-53; exit wloop; endif
            ier=this%loc_list%get_status()
            if(ier.eq.GFC_IT_ACTIVE) then; ier=GFC_SUCCESS; else; if(errc.eq.0) then; errc=-52; exit wloop; endif; endif
            eloop: do while(ier.eq.GFC_SUCCESS)
             uptr=>this%loc_list%get_value(ier); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-51; exit wloop; endif
             tens_instr=>NULL(); select type(uptr); class is(tens_instr_t); tens_instr=>uptr; end select
             if(.not.associated(tens_instr).and.errc.eq.0) then; errc=-50; exit wloop; endif !trap
             if(rot_num.eq.0) then !only deactivate own control instructions, except RESUME
              ...
              if(opcode.ge.TAVP_ISA_CTRL_FIRST.and.opcode.le.TAVP_ISA_CTRL_LAST.and.opcode.ne.TAVP_INSTR_CTRL_RESUME) then !control instructions do not need locating (but keep RESUME)
               ...
               ier=this%loc_list%move_elem(this%ctrl_list); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-48; exit wloop; endif
               cycle eloop
              endif
             endif
             ...
             do i=1,n
              ...
              if(tens_cache_entry%holds_remote().and.tens_cache_entry%get_ref_count().eq.0) then
               ...
               if(errc.eq.0) then
                ...
                if((.not.evicted).and.(errc.eq.0)) errc=-33 !trap
               endif
              endif
             enddo
!$OMP END CRITICAL (TAVP_MNG_CACHE)
  ...
  !Synchronize the bytecode send and clean bytecode buffer:
  ...
  !Absorb located tensor insructions from port 1 (delivered by ldecoder):
   !Wait until port 1 is filled in by ldecoder:
            ...
            do while(ier.eq.GFC_IT_EMPTY) !wait until located tensor instructions are delivered by ldecoder
             ...
             ier=this%loc_list%get_status()
            enddo
   !Read the first dummy instruction with bytecode tag written in its id:
            ier=this%loc_list%reset(); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-25; exit wloop; endif
            ier=this%loc_list%get_status()
            if(ier.eq.GFC_IT_ACTIVE) then
             ...
             if(opcode.eq.TAVP_INSTR_CTRL_RESUME) then !dummy instruction is RESUME
  ...
  !Check whether the newly arrived tensor instructions belong to me (thus finish the rotation):
            if(bytecode_tag.eq.role_rank) exit rloop
           enddo rloop
          endif !ring exists
 !Move partially located tensor instructions from the locating list to the deferred list:
          ...
          enddo
          if((ier.ne.GFC_NO_MOVE.and.ier.ne.GFC_IT_EMPTY).and.errc.eq.0) then; errc=-10; exit wloop; endif
 !Append saved control instructions at the end of the locating list:
          ier=this%loc_list%reset_back(); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-9; exit wloop; endif
          ier=this%ctrl_list%reset(); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-8; exit wloop; endif
          ier=this%ctrl_list%get_status()
          if(ier.eq.GFC_IT_ACTIVE) then
           ier=this%ctrl_list%move_list(this%loc_list); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-7; exit wloop; endif
          endif
 !Move located tensor instructions plus control instructions from the locating list to the decomposer:
          ier=this%loc_list%reset(); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-6; exit wloop; endif
          ier=this%loc_list%get_status()
          if(ier.eq.GFC_IT_ACTIVE) then
           ier=tavp%decomposer%load_port(0,this%loc_list); if(ier.ne.DSVP_SUCCESS.and.errc.eq.0) then; errc=-5; exit wloop; endif
          endif
...
!Initialize the creating list iterator:
         ier=this%crt_list%init(this%create_list); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) errc=-1
...
!Deactivate the creating list:
         ier=this%crt_list%reset()
         if(ier.eq.GFC_SUCCESS) then
          ier=this%crt_list%get_status()
          if(ier.ne.GFC_IT_EMPTY) then
           if(errc.eq.0) errc=-7
           ier=this%crt_list%delete_all()
          endif
          ier=this%crt_list%release(); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) errc=-6
         else
          if(errc.eq.0) errc=-5
         endif
...
!placed into the subinstruction list, prepending the subsequently generated subinstructions.
...
!assessing function and the corresponding threshold for tensor dimension splitting.
         implicit none
         class(tavp_mng_decomposer_t), intent(inout):: this !inout: TAVP-MNG decomposer DSVU
...
!Decompose the tensor instruction:
            if(errc.eq.0) then
             ...
             if(errc.eq.DSVP_SUCCESS) then
              ...
              if(errc.eq.DSVP_SUCCESS) then
               select case(opcode)
 !TENSOR CREATE:
               case(TAVP_INSTR_TENS_CREATE) !new TENSOR_CREATE subinstructions have already been appended into the creating list
 !TENSOR DESTROY:
 ...
 !TENSOR CONTRACT:
        ...
        contains
         ...
         !In case the output operand(s) do not have internal structure yet,
         ...
         !For each new subtensor, a tensor instruction TENSOR_CREATE
         !is constructed and appended into the creating list.
         !`Identical output tensor operands may appear on different TAVPs causing duplication.
          implicit none
          ...
          if(jerr.eq.GFC_SUCCESS) then
           tloop: do jj=0,tens_instr%num_out_oprnds-1 !loop over the output tensor operands
            ...
            if(jerr.ne.DSVP_SUCCESS) then; jerr=-24; exit tloop; endif
            select type(oprnd)
            class is(tens_oprnd_t)
             tensor=>oprnd%get_tensor(jerr); if(jerr.ne.0) then; jerr=-23; exit tloop; endif
             if(tensor%get_num_subtensors().le.0) then !tensor does not have an internal structure yet
              ...
              if(jerr.ne.0) then; jerr=-22; exit tloop; endif
!$OMP CRITICAL (TAVP_MNG_CACHE)
              call tensor%decompose(split_dims(1:ns),jerr)
!$OMP END CRITICAL (TAVP_MNG_CACHE)
              if(jerr.ne.TEREC_SUCCESS) then; jerr=-21; exit tloop; endif
              subtensors=>tensor%get_subtensors(jerr); if(jerr.ne.TEREC_SUCCESS) then; jerr=-20; exit tloop; endif
              jerr=lit%init(subtensors); if(jerr.ne.GFC_SUCCESS) then; jerr=-19; exit tloop; endif
              cloop: do while(jerr.eq.GFC_SUCCESS)
               uptr=>lit%get_value(jerr); if(jerr.ne.GFC_SUCCESS) then; jerr=-18; exit tloop; endif
               header=>NULL(); select type(uptr); class is(tens_header_t); header=>uptr; end select
               if(.not.associated(header)) then; jerr=-17; exit tloop; endif
               allocate(subtensor,STAT=jerr); if(jerr.ne.0) then; jerr=-16; exit tloop; endif
               call subtensor%tens_rcrsv_ctor(header,jerr); if(jerr.ne.TEREC_SUCCESS) then; jerr=-15; exit tloop; endif
!$OMP CRITICAL (TAVP_MNG_CACHE)
               stored=this%arg_cache%store(subtensor,tens_entry_mng_alloc,jerr,tens_entry_p=tens_entry) !tensor ownership is moved to the tensor cache entry
!$OMP END CRITICAL (TAVP_MNG_CACHE)
               if(jerr.eq.0.and.stored.and.associated(tens_entry)) then
                tens_entry_mng=>NULL(); select type(tens_entry); class is(tens_entry_mng_t); tens_entry_mng=>tens_entry; end select
                if(.not.associated(tens_entry_mng)) then; jerr=-14; exit tloop; endif
                jerr=this%crt_list%append(tens_instr_empty); if(jerr.ne.GFC_SUCCESS) then; jerr=-13; exit tloop; endif
                jerr=this%crt_list%reset_back(); if(jerr.ne.GFC_SUCCESS) then; jerr=-12; exit tloop; endif
                uptr=>this%crt_list%get_value(jerr); if(jerr.ne.GFC_SUCCESS) then; jerr=-11; exit tloop; endif
                subinstr=>NULL(); select type(uptr); class is(tens_instr_t); subinstr=>uptr; end select
                if(.not.associated(subinstr)) then; jerr=-10; exit tloop; endif !trap
                iid=dsvp%get_crtd_instr_counter()
                call subinstr%tens_instr_ctor(TAVP_INSTR_TENS_CREATE,jerr,subtensor,iid)
                if(jerr.ne.0) then; jerr=-9; exit tloop; endif
                tens_oprnd=>subinstr%get_operand(0,jerr); if(jerr.ne.DSVP_SUCCESS) then; jerr=-8; exit tloop; endif
                select type(tens_oprnd)
                class is(tens_oprnd_t)
                 call tens_oprnd%set_cache_entry(tens_entry_mng,jerr); if(jerr.ne.0) then; jerr=-7; exit tloop; endif
                class default
                 jerr=-6; exit tloop
                end select
                call dsvp%incr_crtd_instr_counter()
                subtensor=>NULL() !tensor ownership has been transferred to the tensor cache
               else
                deallocate(subtensor); jerr=-5; exit tloop
               endif
               jerr=lit%next()
              enddo cloop
              if(jerr.ne.GFC_NO_MOVE) then; jerr=-4; exit tloop; endif
              jerr=lit%release(); if(jerr.ne.GFC_SUCCESS) then; jerr=-3; exit tloop; endif
             endif
            class default
             jerr=-2; exit tloop
            end select
         ...
         !appended to the subinstruction list.
          implicit none
          integer(INTD), intent(out):: jerr !out: error code
          integer(INTD):: jj
          integer(INTL):: iid
            ...
            class is(tens_oprnd_t)
             ...
             if(jerr.eq.0) then
              if(tensor%get_num_subtensors().gt.0) then !tensor must have an internal structure
               subtensors=>tensor%get_subtensors(jerr)
               if(jerr.eq.TEREC_SUCCESS) then
                jerr=lit%init(subtensors)
                if(jerr.eq.GFC_SUCCESS) then
                 cloop: do while(jerr.eq.GFC_SUCCESS)
                  uptr=>lit%get_value(jerr); if(jerr.ne.GFC_SUCCESS) then; jerr=-1; exit cloop; endif
                  header=>NULL(); select type(uptr); class is(tens_header_t); header=>uptr; end select
                  if(.not.associated(header)) then; jerr=-1; exit cloop; endif
                  allocate(subtensor,STAT=jerr); if(jerr.ne.0) then; jerr=-1; exit cloop; endif
                  call subtensor%tens_rcrsv_ctor(header,jerr); if(jerr.ne.TEREC_SUCCESS) then; jerr=-1; exit cloop; endif
                  tens_entry=>NULL(); tens_entry=>this%arg_cache%lookup(subtensor,jerr) !subtensor must be present in the tensor cache since its creation
                  if(.not.associated(tens_entry)) then; jerr=-1; exit cloop; endif !trap
                  deallocate(subtensor); subtensor=>tens_entry%get_tensor(jerr); if(jerr.ne.0) then; jerr=-1; exit cloop; endif
                  tens_entry_mng=>NULL()
                  select type(tens_entry); class is(tens_entry_mng_t); tens_entry_mng=>tens_entry; end select
                  if(.not.associated(tens_entry_mng)) then; jerr=-1; exit cloop; endif
                  jerr=this%sub_list%append(tens_instr_empty); if(jerr.ne.GFC_SUCCESS) then; jerr=-1; exit cloop; endif
                  jerr=this%sub_list%reset_back(); if(jerr.ne.GFC_SUCCESS) then; jerr=-1; exit cloop; endif
                  uptr=>this%sub_list%get_value(jerr); if(jerr.ne.GFC_SUCCESS) then; jerr=-1; exit cloop; endif
                  subinstr=>NULL(); select type(uptr); class is(tens_instr_t); subinstr=>uptr; end select
                  if(.not.associated(subinstr)) then; jerr=-1; exit cloop; endif !trap
                  iid=dsvp%get_crtd_instr_counter()
                  call subinstr%tens_instr_ctor(TAVP_INSTR_TENS_DESTROY,jerr,subtensor,iid)
                  if(jerr.ne.0) then; jerr=-1; exit cloop; endif
                  tens_oprnd=>subinstr%get_operand(0,jerr); if(jerr.ne.DSVP_SUCCESS) then; jerr=-1; exit cloop; endif
                  select type(tens_oprnd)
                  class is(tens_oprnd_t)
                   call tens_oprnd%set_cache_entry(tens_entry_mng,jerr); if(jerr.ne.0) then; jerr=-1; exit cloop; endif
                  class default
                   jerr=-1; exit cloop
                  end select
                  call dsvp%incr_crtd_instr_counter()
                  ...
                  jerr=lit%next()
                 enddo cloop
                 if(jerr.ne.GFC_NO_MOVE) jerr=-1
                 jj=lit%release(); if(jj.ne.GFC_SUCCESS.and.jerr.eq.0) jerr=-1
                else
                 jerr=-1
                endif
               else
