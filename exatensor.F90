...
!PARAMETERS:
 ...
 !Elementary tensor instruction (ETI) granularity:
        real(4), public:: FLOPS_MEDIUM=1d9 !minimal number of Flops to consider the operation as medium-cost
        real(4), public:: FLOPS_LARGE=1d10 !minimal number of Flops to consider the operation as large-cost
        real(4), public:: COST_TO_SIZE=1d1 !minimal cost to size ratio to consider the operation cost-efficient
 !Tensor algebra virtual processor:
  ...
  !Tensor instruction status (any negative value will correspond to a failure, designating the error code):
        ...
        integer(INTD), parameter, public:: INSTR_FRESH=1         !instruction has not been seen before (new instruction)
        ...
        integer(INTD), parameter, public:: INSTR_SCHEDULED=4     !instruction is placed into an execution queue on a specific CU
        integer(INTD), parameter, public:: INSTR_ISSUED=5        !instruction is issued for execution to some computing unit (CU)
        integer(INTD), parameter, public:: INSTR_COMPLETED=6     !instruction has completed (but the result may still need a remote upload)
        integer(INTD), parameter, public:: INSTR_DEAD=7          !instruction can safely be removed from the queue
